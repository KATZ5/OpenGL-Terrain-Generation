...
vec3 noised( in vec2 x )
{
    ...
    float a = texelFetch( ourTexture, (p+ivec2(0,0))&255, 0 ).x;
    float b = texelFetch( ourTexture, (p+ivec2(1,0))&255, 0 ).x;
    float c = texelFetch( ourTexture, (p+ivec2(0,1))&255, 0 ).x;
    float d = texelFetch( ourTexture, (p+ivec2(1,1))&255, 0 ).x;
    #else
    // texture version
    vec2 p = floor(x);
    float a = textureLod( ourTexture, (p+vec2(0.5,0.5))/256.0, 0.0 ).x;
    float b = textureLod( ourTexture, (p+vec2(1.5,0.5))/256.0, 0.0 ).x;
    float c = textureLod( ourTexture, (p+vec2(0.5,1.5))/256.0, 0.0 ).x;
    float d = textureLod( ourTexture, (p+vec2(1.5,1.5))/256.0, 0.0 ).x;
    #endif
...
{
    float f = 0.0;
    f += 0.5000*texture( ourTexture, p/256.0 ).x; p = m2*p*2.02;
    f += 0.2500*texture( ourTexture, p/256.0 ).x; p = m2*p*2.03;
    f += 0.1250*texture( ourTexture, p/256.0 ).x; p = m2*p*2.01;
    f += 0.0625*texture( ourTexture, p/256.0 ).x;
    return f/0.9375;
}
...
vec4 render( in vec3 ro, in vec3 rd )
{
    ...
    else
    {
        ...
        // rock
        float r = texture( ourTexture, (7.0/SC)*pos.xz/256.0 ).x;
        col = (r*0.25+0.75)*0.9*mix( vec3(0.08,0.05,0.03), vec3(0.10,0.09,0.08),
                                     texture(ourTexture,0.00007*vec2(pos.x,pos.y*48.0)/SC).x );
...
void main()
{
    ...
    float time = iTime*0.1 - 0.1 + 0.3 + 4.0;
